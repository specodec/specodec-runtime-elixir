...
run(`cd ${__dir} && node_modules/.bin/tsp compile ${CACHE}/alltypes.tsp --emit=@specodec/typespec-emitter-elixir --option @specodec/typespec-emitter-elixir.emitter-output-dir=${GENERATED}`);

console.log("\n=== Step 3: Run roundtrip tests ===");
if (existsSync(OUT_DIR)) rmSync(OUT_DIR, { recursive: true });
ensure(OUT_DIR);
run(`cd ${RUNTIME} && mix clean 2>/dev/null; mix deps.get 2>/dev/null; mix compile 2>/dev/null`);
const VEC_DIR_ABS = join(__dir, ".tests-cache", "vectors");
